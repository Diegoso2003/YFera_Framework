...
export class Clase extends NodoEstilo {
  ...
  añadirHeredado(analizador: AnalizadorStyles) {
    ...
    if (this.nombre.getPadre()) {
      const clase = analizador.getClases().get(this.nombre.getPadre());
      if (!clase) {
        errores.agregarSimbolo(this.nombre.getSimbolo(), `no se encontro la clase padre '${this.nombre.getPadre()}'`);
      } else {
        clase.compilado.forEach((compilado, estilo) => {
